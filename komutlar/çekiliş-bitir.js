...
exports.run = async (client, message, args) => {
 if(!message.member.permissions.has('ADMINISTRATOR')) {
    ...
    if(!args[0]){
        return message.channel.send('Bir çekiliş belirtmelisin!');
    }
    ...
    .catch((e) => {
        if(e.startsWith(`Bu ID çekiliş kimliği ${giveaway.messageID} zaten bitti.`)){
            message.channel.send('Bu çekiliş sona erdi!');
        } else {
            console.error(e);
